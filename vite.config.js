  ...
  plugins: [
    ...
    },
    {
      ...
      async generateBundle() {
        const inputPath = path.resolve('./src/App.jsx');
        const outputDir = path.resolve('./build');

        const transformFile = async (filePath) => {
          const code = fs.readFileSync(filePath, 'utf-8');
          const updatedCode = code.replace(/(import\s+.*?['"])(.*?)(['"])/g, (match, p1, p2, p3) => {
            if (p2.startsWith('.') || p2.startsWith('/')) {
              if (fs.existsSync(path.resolve(path.dirname(filePath), p2 + '.jsx'))) {
                return `${p1}${p2}.js${p3}`;
              } else if (fs.existsSync(path.resolve(path.dirname(filePath), p2 + '.js'))) {
                return `${p1}${p2}.js${p3}`;
              }
            }
            return match;
          ...
          const { code: transformedCode } = await transform(updatedCode, { loader: 'jsx', format: 'esm' });
          const relativePath = path.relative(path.resolve('./src'), filePath);
          const outputPath = path.join(outputDir, relativePath).replace(/\.jsx$/, '.js');
